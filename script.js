	...
	function playerCalc() {
		...
		document.addEventListener("DOMContentLoaded", function () {
			...
			function handleOrientation(event) {
				...
				const beta = event.beta;   // Наклон вперёд/назад (от -180 до 180)
		
				// Обработка наклона влево/вправо
				if (gamma < -10) {
					dir = "left";
				...
				// Обработка наклона вперёд/назад
				if (beta < -10) {
					dir = "up";
				...
				// Для проверки выводим текущее направление
				console.log("Direction:", dir, "Player state:", player);
			}
		
			// Проверяем доступность событий ориентации
			if (window.DeviceOrientationEvent) {
				// Для iOS требуется запросить разрешение
						...
						DeviceOrientationEvent.requestPermission()
							.then((permissionState) => {
								if (permissionState === "granted") {
									...
									alert("Разрешение на доступ к ориентации устройства не предоставлено.");
								}
							})
							.catch(console.error);
					});
				...
				alert("Ваше устройство не поддерживает отслеживание ориентации.");
			}
		});
